...
link_module_df.to_csv("link_module_count.csv", index=False)
...
demand_paths_df.to_csv("demand_paths.csv", index=False)

# Zapis demand_volume do CSV
demand_volume_df = pd.DataFrame(list(demand_volume.items()), columns=["Demand", "Volume"])
demand_volume_df.to_csv("demand_volume.csv", index=False)
